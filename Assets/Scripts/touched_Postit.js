...
if (Input.GetMouseButtonDown(0) && cube.doubleclickCounter%2==1) {
   ...
   /* if(GuiWrite.writtenUpon /*&& closedScript.closed){
    hit.collider.gameObject.renderer.material.SetTexture("_MainTex", scribbleText);

}

    else if(!GuiWrite.writtenUpon /*&& closedScript.closed){
	hit.collider.gameObject.renderer.material.SetTexture("_MainTex", regularYel);
